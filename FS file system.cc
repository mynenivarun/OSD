...
// struct superblock sb; // there should be one per dev, but 
we run with one dev
struct mbr mbr;
...
bootable = yes;
boot_partition = checkForBootPrograms(i); // mark first 
bootable partition as boot partition
}
else {
bootable = no;
}
...
cprintf("Partition %d: bootable: %s, type:%s, offset:%d, 
size:%d \n", i, bootable, type, mbr->partitions[i].offset, mbr-
>partitions[i].size);
// memmove(&partitions[i] + sizeof(uint), &mbr-
>partitions[i], sizeof(struct dpartition));
current_partition = i;
...
bp = bread(dev, BBLOCK(b, sbs[current_partition]) + 
partitions[current_partition].offset);
for(bi = 0; bi < BPB && b + bi < sbs[current_partition].size; 
bi++){
m = 1 << (bi % 8);
...
// pathname lookup. iget() increments ip->ref so that the 
inode
...
readmbr(dev, &mbr);
// readsb(dev, &sbs[0]);
cprintf("Boot partition: size %d nblocks %d ninodes %d nlog 
%d logstart %d inodestart %d bmap start %d\n", 
sbs[boot_partition].size,
sbs[boot_partition].nblocks, sbs[boot_partition].ninodes, 
sbs[current_partition].nlog, sbs[boot_partition].logstart, 
sbs[boot_partition].inodestart, 
sbs[boot_partition].bmapstart);
...
struct buf *bp;
struct dinode *dip;
for(inum = 1; inum < sbs[current_partition].ninodes; 
inum++){
bp = bread(dev, IBLOCK(inum, sbs[current_partition]) + 
partitions[current_partition].offset);
...
struct buf *bp;
struct dinode *dip;
bp = bread(ip->dev, IBLOCK(ip->inum, sbs[ip->partition]) + 
partitions[ip->partition].offset);
...
empty = 0;
for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; 
ip++){
if(ip->ref > 0 && ip->dev == dev && ip->inum == inum && ip-
...
// NOTE during testing, check if balloc returns a relative block 
number
...
if(ip->addrs[NDIRECT]){
bp = bread(ip->dev, ip->addrs[NDIRECT] + partitions[ip-
>partition].offset);
a = (uint*)bp->data;
...
struct buf *bp;
if(ip->type == T_DEV){
if(ip->major < 0 || ip->major >= NDEV || !devsw[ip-
>major].read)
return -1;
...
bp = bread(ip->dev, bmap(ip, off/BSIZE) + partitions[ip-
>partition].offset);
...
if (mapping[i][0].inum == inum && mapping[i][0].partition == 
partition) {
...
// so the caller can check *path=='\0' to see if the name is the 
last one.
...
// If parent != 0, return the inode for the parent and copy the 
final
// path element into name, which must have room for DIRSIZ 
bytes.
...
//ip is the inode which is being mapped from, 
partition_number is the partition being mounted to
int
insert_mapping(struct inode * ip,int partition_number){
int i;
for (i = 0; i < NPARTITIONS * MAXNUMINDOES; i++) {
if (mapping[i][0].inum == ip->inum && mapping[i][0].partition 
== ip->partition) {
