...
// creates a thread, passes the address of two 
variables
// calls join to wait on the thread
// thread swaps the values of the two variables 
(shared memory)
// if values are swapped when main thread comes out 
of join(), test OK
int
jointest(void)
{
...
if (buffer == (char *)-1)
return -1;
retval = clone(jointestchild, (void *)&a, (void
*)&b, buffer + 4096, 0);
join(retval);
sbrk(-4096);
if (a == 25 && b == 42)
...
// creates two threads using clone, the first one 
sleeps for 100ms
// second one sleeps for 50ms.
// join is called on the first thread. join should 
return on first thread
// even if the second thread exits first
// also ensures that wait is not cleaning up before 
join
int
jointest1(void)
{
...
one = 100;
two = 50;
tid1 = clone(jointestchild1, (void *)&one, 0, 
buffer1 + 4096, 0);
tid2 = clone(jointestchild1, (void *)&two, 0, 
buffer2 + 4096, 0);
ret1 = join(tid1);
ret2 = join(tid2);
...
stack11 = sbrk(4096);
stack12 = sbrk(4096);
tid11 = clone(jointestchild1, &one1, 0, stack11 + 
4096, 0);
tid12 = clone(jointestchild1, &two1, 0, stack12 + 
4096, 0);
ret11 = join(tid11);
ret12 = join(tid12);
sbrk(-8192);
...
printf(1, "Wait reaped before join, wait-join 
exclusivity test failed\n");
exit();
}
tgid2 = fork();
if (!tgid2){
...
stack21 = sbrk(4096);
stack22 = sbrk(4096);
tid21 = clone(jointestchild1, &one2, 0, stack21 + 
4096, 0);
tid22 = clone(jointestchild1, &two2, 0, stack22 + 
4096, 0);
ret21 = join(tid21);
ret22 = join(tid22);
sbrk(-8192);
...
// process forks
// int he child process, two threads are created, 
one dummy thread, and one
// which exec()s
// the child process tries to *join* the thread 
which calls exec.
// this join should fail as all threads should be 
terminated by exec
// the parent process waits. wait should return the 
pid of the child process
// because the exec() should transfer the pid to 
the thread calling exec()
int
exectest(void)
{
int ret1, ret2;
...
stack1 = sbrk(4096);
stack2 = sbrk(4096);
tid1 = clone(jointestchild1, &thousand, 0, stack1 + 
4096, 0);
...
// meant to test that sbrk() work properly in 
multithreaded env
// process creates two threads, which sbrk(1) in a 
loop, 100 times
// process joins the two threads
// the process size after the join should be 
initial process size + 200
// (if sz were not protected by a lock, there might 
be a race, which will mean
...
// so there are 61 remaining slots after init, sh, 
clonetests
...
for (i = 0; i < NT; i++){
arr[i] = i;
if((ret = cthread_create(&threads[i], cottonthread, 
(void *)&arr[i], 0)) == -1){
...
// this was for weeding out a specific issue where 
two threads called exec() at
// the same time, one of them would *not* get 
killed
int
twoexectest(void)
{
...
ret = cthread_create(&arr[i], jointestchild1, 
&time, 0);
if(ret == -1){
break;
}
...
// threads should print their PID in the order 
which they were spawned (ideally)
// could not think of a way to test this, could 
verify visually.
#define NT 61
int
tickettest(void)
...
for (i = 0; i < NT; i++){
arr[i] = i;
cthread_create(&threads[i], cottonticket, (void
*)&arr[i], (void *)&lock);
}
for (i = 0; i < NT; i++){
...
// 61 threads race on incrementing a single integer 
1000 times each, with access
// syncrhonised using a lock
// the value of the variable after joining should 
be (1000 * NT)
int
racetest(void){
cthread_t threads[NT];
...
slock_init(&lock);
for (i = 0; i < NT; i++){
cthread_create(&threads[i], incracer, (void *)&val, 
(void *)&lock);
}
for (i = 0; i < NT; i++){
cthread_join(&threads[i]);
...
// child should be killed before it changes the 
value of the flag
int
childkilltest(void)
{
...
int value = 0;
int pid;
pid = cthread_create(&child, wayvard_child, (void
*)count, (void *)value);
kill(pid);
join(pid);
if(value)
...
// T2 checks arguments of a system call, finds them 
to be fine
// T1 decreases the process size
// T2 dereferences the argument (which is a 
buffer), and gets killed due to out
// of bounds access
struct baton{
int turn;
slock_t lock;
...
// sets flag and immediately sbrk's the buffer out 
of the memory space
// read should fail in the child.
// yields() have been added in the kernel which 
cause wrong access when safe copy primitives
// are not used
int
vmsynctest(void)
{
...
ret = clone(vmmessfunc, (void *)&flag, (void *)buf, 
stack + 4096, 0);
flag = 1;
sbrk(-BUFFERSIZE);
join(ret);
...
// buffer which spans multiple pages in heap 
created, ands set to some value
// the buffer should have the same value in the 
child
int
vmemtest(void){
char *buffer;
...
char ch = 0;
cthread_t thread;
cthread_create(&thread, checker, (void *)buffer, 
(void *)&ch);
cthread_join(&thread);
if(!ch)
...
// the child creates a directory, changes working 
directory
// creates a file in the new working directory, 
writes bytes to it and exits
// the parent joins the child thread. the parent 
...
// similar to the filewrite safe copy primitives 
test, except for pipes
// not using safe copy primitives causes TOCTOU 
panics in the kernel
int
pipevmsynctest(void)
{
char *stack = sbrk(4096);
...
ret = clone(clonevmmessfunc, (void *)&flag, (void
*)&cont, stack + 4096, 0);
flag = 1;
...
// create children which repeatedly access the same 
location in a page allocated
// in the parent resulting in a tlb entry. Then 
sets a flag
// the parent on seeing the flag invalidates the 
page, and sets another flag
// which causes the thread to proceed, and try to 
access the same location
// this should kill the thread, and not allow it to 
proceed, (because the TLB
// entry was invalidated)
// this test fails, as of now. too many 
complications in using multiprocessor
// interrupts
#define TLB_N 1
#define REPS 1
int
...
char *page = sbrk(4096);
for(i = 0; i < TLB_N; i++){
pid[i] = clone(tlb_child, (void *)&bt, (void
*)&page[i], (void *)pages[i] + 4096, 0);
}
while(bt.threadsready != TLB_N)
;
/* children have loaded TLB, now invalidate the 
page */
sbrk(-4096);
/* allow threads to access the page */
bt.parentready = 1;
/* wait for threads to die or to access and 
increment */
sleep(5);
if(!bt.threadsready){
...
int var = 0;
int other = 0;
cthread_create(&child, condvarchild, (void *)&var, 
(void *)&other);
var = 0;
sleep(50);
if(other){
...
int var = 0;
int other = 0;
cthread_create(&child, lostchild, (void *)&var, 
(void *)&other);
unpark(child.pid, (void *)&var);
cthread_join(&child);
...
// for testing the implementation of the queue used 
in the semaphore, checks
// FIFO
int
queuetest(void)
{
...
// the item created by the producer should be the 
one recieved by the consumer
// otherwise, simply sees that there are no 
deadlocks happening due to lost
// wakeups
int
...
cthread_create(&cons[i], consumer, (void *)&slots, 
(void *)&items);
for(i = 0; i < THREADSN; i++)
cthread_create(&prod[i], producer, (void *)&slots, 
(void *)&items);
for(i = 0; i < THREADSN; i++){
cthread_join(&prod[i]);
cthread_join(&cons[i]);
...
// no deadlocks in dining philosophers and everyone 
is able to eat
// (as the while loop is not infinite, there will 
not be starvation anyway)
int
